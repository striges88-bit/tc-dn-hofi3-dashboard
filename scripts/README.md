...
Use `memory-semantic-doctor.ps1` before the LanceDB semantic gate on a fresh or repaired machine. It checks `uv` discovery, pinned local semantic dependencies (`lancedb==0.34.0`, `pyarrow==24.0.0`, `fastembed==0.8.0`), cache/venv policy, and offline runtime availability. Gate commands use `uv --offline`; hidden network downloads are not allowed inside `memory-refresh-all`, `memory-pre-push-check`, or hooks. Any dependency/model download must be an explicit preflight and must keep caches outside the repository.
