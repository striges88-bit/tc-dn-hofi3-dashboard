...
- The current LanceDB candidate uses local FastEmbed/ONNX by default with model `sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2`, wrapper pin `fastembed==0.8.0`, and `embedding_pooling_baseline=mean-pooling`. This baseline is accepted only while LanceDB `eval` passes `9/9`; changing package, model, provider, dimensions, or pooling requires rerun cleanup/rebuild/eval and an updated baseline report.
...
- LanceDB `eval` must write compact generated JSON/Markdown reports with query, expected ids/types, matched rank, source path, confidence, gap notes, FastEmbed package/model metadata, `embedding_pooling_baseline`, and the baseline eval gate. These reports are review evidence only; they do not override SQLite status, ADRs, formulas, or source code.
