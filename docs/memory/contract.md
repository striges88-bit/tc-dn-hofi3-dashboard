...
Commit-addressed semantic evidence includes `docs/memory/generated/lancedb-manifest.json`. Rebuild is its only producer. The manifest contract requires exact `schema_version=1`, `generator=tools/MemorySemantic/lancedb_sidecar.py`, `status=ready`, `source_store=sqlite-fts5`, `lancedb_table=memory_documents`, a non-negative JSON integer `indexed_count`, non-empty `commit_sha`, `tree_sha`, and `indexed_at`, plus the exact embedding provider/model/runtime/package/dimensions/pooling identity. Rebuild must compare `indexed_count` with the created table's physical row count before publishing the manifest. Search, explain, and eval must compare it again after opening the table and before embedding or query execution. Manual pre-push and CI must reject a missing store, malformed manifest, stale commit/tree, or embedding mismatch before trusting semantic results.

Git lookup failures in the manual gate use structured evidence rather than only message text: `operation`, `failure_code`, JSON boolean `timed_out`, integer `timeout_ms`, nullable integer `exit_code`, and diagnostic `error`. Stable failure codes are `git-unavailable`, `git-timeout`, and `git-exit-nonzero`. CI and local gates must fail closed when Git identity is unavailable; they must not reinterpret missing evidence as a fresh index.
...
Use `scripts/memory-semantic-doctor.ps1` before relying on the LanceDB semantic gate on a fresh or repaired machine. It is a local dependency preflight for `uv`, `lancedb==0.34.0`, `pyarrow==24.0.0`, `fastembed==0.8.0`, cache policy, and offline runtime availability. The memory gate path must use offline `uv` execution and block hidden network downloads; dependency or model downloads require an explicit preflight and must keep cache/model state outside the repo.
...
- The LanceDB semantic quality gate must cover current OFI formula retrieval, formula owner retrieval, funding-source decision retrieval, Binance DTO boundary retrieval, REST hot-path ban retrieval, live/replay shared-pipeline retrieval, funding slow-context retrieval, exchange adapter impact retrieval, strict no-answer for superseded/failed-only facts, and no-answer/low-confidence behavior before any automation is added.
- Keep `tasks/todo.md` in canonical SQLite/FTS for exact operational retrieval, but do not copy its generic `chunk` records into LanceDB: completed handoffs and eval diagnostics are noisy semantic evidence. Future typed `todo` records remain eligible for the sidecar, and durable `tasks/lessons.md` chunks remain semantic sources.
- LanceDB `eval` must write compact generated JSON/Markdown reports with query, expected ids/types, matched rank, source path, confidence, gap notes, FastEmbed package/model metadata, `embedding_runtime_model`, `embedding_pooling`, `embedding_pooling_baseline`, `embedding_warning_policy`, and the baseline eval gate. Typed facts use `minimum_retrieval_confidence=0.40`; generic chunks use `minimum_chunk_retrieval_confidence=0.50`. LanceDB `search` and `explain` must report both floors, each result's effective `retrieval_confidence_threshold`, `freshness_check`, returned count, and gap notes so weak or stale matches are visible instead of silently becoming answers. These reports are review evidence only; they do not override SQLite status, ADRs, formulas, or source code.
...
- Use `scripts/curated-retain-dry-run.ps1` before any future retain implementation to produce provider-neutral JSON/Markdown redaction reports from the same allowlist. The reports are generated evidence only and must not import, retain, rebuild, install hooks, or call external providers.
