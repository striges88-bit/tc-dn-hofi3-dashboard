...
Use `scripts/curated-retain-import.ps1` only for controlled local import after dry-run review or reviewed redacted-subset generation. It allowlists schema-1 `dry-run` and schema-2 `redacted-subset` contracts, requires the exact known generator plus present, correctly typed safety flags, and fails closed on incomplete, review-required, blocked, failed, unknown, or explicitly blocked reports. Every source must be a canonical repository-relative allowlist path with no rooted, `.` or `..` segments, and Git blob lookup must verify that the source exists in the selected commit. It wraps SQLite `retain-import`, reads clean candidates from the selected Git commit tree, and stores reviewed redacted text for redacted entries while preserving commit/source metadata. It writes an ignored generated report and must not call external retain providers, Codex memory, Cloud, hooks, LanceDB rebuild, raw JSONL, generated exports, secrets, or build artifacts.
...
- LanceDB `eval` must write compact generated JSON/Markdown reports with query, expected ids/types, matched rank, source path, confidence, gap notes, FastEmbed package/model metadata, `embedding_runtime_model`, `embedding_pooling`, `embedding_pooling_baseline`, `embedding_warning_policy`, and the baseline eval gate. LanceDB `search` and `explain` must include `freshness_check`, `minimum_retrieval_confidence`, returned count, and gap notes so weak or stale matches are visible instead of silently becoming answers. These reports are review evidence only; they do not override SQLite status, ADRs, formulas, or source code.
