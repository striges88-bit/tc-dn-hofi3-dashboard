...
Import allowlists only the known report contracts: schema-1 `dry-run` and schema-2 `redacted-subset`. The schema/mode/status tuple is not sufficient by itself: the exact known generator and every required safety flag must be present with the expected JSON type and value. A batch can import only from `ready_for_review` or `ready_for_import`; incomplete metadata, `review_required`, `blocked`, `failed`, unknown/missing status, or any `blocking_reasons` fail closed. Every source must also be an allowlisted canonical repository-relative path without rooted, `.` or `..` segments, exist in the selected Git commit, match its reviewed hash, and be either a redaction-clean `candidate` with zero findings or reviewed `redacted` with `redacted_text`. Denylisted paths, stale hashes, missing source metadata, missing redacted text, or redaction review findings block the whole batch. The generated report is `docs/memory/generated/curated-retain-import-report.json`.
