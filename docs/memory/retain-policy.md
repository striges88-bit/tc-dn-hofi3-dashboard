...
Controlled local import uses `scripts/curated-retain-import.ps1`, which wraps `tools/Memory` command `retain-import`. Import reads source text from the requested Git commit tree, default `HEAD`, and requires a reviewed schema-v2 subset derived from the curated dry-run report. A dirty working tree must not change imported text.

Use `scripts/curated-retain-redacted-subset.ps1 -SourcePath <repo/path.md>` to turn explicitly selected, reviewed dry-run entries into a local subset report. Schema v2 stores a clean `candidate` only as `content_kind=commit-source-reference`; the report does not embed its source text. A source with findings becomes `content_kind=reviewed-redacted-text`, replaces risky lines with `[REDACTED:<finding-types>]`, and includes only reviewed `redacted_text`. Separate content flags make those cases explicit. The script rejects sources changed since the dry-run report, preserves the original source hash for commit verification, records `original_finding_count`, and writes ignored JSON/Markdown reports under `docs/memory/generated/`. It does not import, retain, rebuild, install hooks, call Cloud, or call Codex retain.

Import allowlists only schema-2 `redacted-subset`; schema-1 `dry-run` remains review evidence and cannot be imported directly. The schema/mode/status tuple is not sufficient by itself: the exact known generator and every required safety/content flag must be present with the expected JSON type and value. A batch can import only from `ready_for_import`; incomplete metadata, `review_required`, `blocked`, `failed`, unknown/missing status, or any `blocking_reasons` fail closed. Every source must also be an allowlisted canonical repository-relative path without rooted, `.` or `..` segments, exist in the selected Git commit, match its reviewed hash, and be either a reviewed redaction-clean `candidate` with zero findings or reviewed `redacted` text derived line-for-line from that Git blob. Redaction markers may contain only known scanner finding types. Denylisted paths, stale hashes, missing source metadata, missing redacted text, unknown markers, or redaction review findings block the whole batch. The generated report is `docs/memory/generated/curated-retain-import-report.json`.
