...
Do not add post-commit memory refresh automation. Do not install Git hooks automatically. A future pre-push hook may wrap the manual helper only after the helper has stable evidence checks and an explicit opt-in installer.
