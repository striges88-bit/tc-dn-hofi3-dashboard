...
## Pilot B v3 Issue #46 — preflight rejection and exclusive ownership (active)

- [x] Verify clean worktree `codex/pilot-b-runner-preflight-ownership` at exact `main@9bb9e653`, live #46 readiness, and closed blocker #43; publish lifecycle `STARTED`.
- [x] Add focused RED tests for typed immutable rejection, every initially existing artifact-path kind, strict manifest fields, and atomic ownership race behavior.
- [x] Implement the minimal fail-closed preflight/ownership boundary without changing started-run, seal, cancellation, controlled-invalid, scorer, protocol/schema, WPF, or real CLI/auth behavior.
- [x] Run focused and full Release tests, Release build, whitespace/scope checks, and adversarial two-axis review.
- [x] Synchronize directly verified #46 acceptance criteria and publish lifecycle `VERIFIED` or `PARTIAL`; stop before stage, commit, push, PR, merge, or Issue closure.

Preflight evidence: target worktree was created from fetched exact remote `main=9bb9e653c9874854805a14a9441cbb10be4f15c6`; branch and worktree were clean; Issue #46 was OPEN and `ready-for-agent`; blocker #43 was CLOSED/completed. Baseline Release `PilotBRunnerTests`: 27/27 passed. `CONTEXT.md` is absent from the exact merged tree, so current code/tests, Issue #46, and the focused harness remain authoritative.

RED evidence: the typed-exception slice first failed to compile because the contract did not exist; existing-path coverage then failed for empty directory, file, and directory reparse; strict-manifest coverage failed 5/11 for case-variant arm IDs and whitespace repository roots; deterministic ownership coverage failed to compile before the concrete boundary existed. The first concurrent-run attempt could observe initial-path rejection instead of the intended post-preflight race, so its atomic primitive was isolated at the concrete real-filesystem ownership boundary. Adversarial review then required end-to-end runner proof; a RED public-`RunAsync` race now uses a per-runner, non-global checkpoint to release both completed preflights together. Current GREEN checkpoints: focused `PilotBRunnerPreflightTests` 14/14 and existing `PilotBRunnerTests` 27/27. Review also preserved underlying I/O/ACL and cleanup diagnostics and consolidated duplicate fixture plumbing into one isolated-temp test helper.
...
## Pilot B v3 Issue #45 — atomic seal and closed artifact verification (active)

- [x] Create the clean `codex/pilot-b-runner-atomic-seal-verification` worktree at exact `main@87e0f952`, confirm #45 readiness and closed blocker #43, restore test assets, pass the 27/27 focused baseline, and publish lifecycle `STARTED`.
- [x] Add RED→GREEN concrete real-filesystem coverage for create-new payload writes, closed pre-seal inventory, temp/final seal collisions, atomic rename, abandoned lock/temp, malformed/partial/unsupported seal, and post-seal mutation.
- [x] Extract one internal `IEvidenceBundlePublisher`, inject it into `PilotBRunner`, and prove all eight runner publication-call failures plus a successful-but-tampered publication return only `UNSEALED + null`.
- [x] Preserve the independent verifier as the sole sealing authority and directly reject exact-inventory violations, traversal, non-regular/reparse entries, Windows hard links, inconsistent final state, length/hash drift, and add/delete/replace/mutate tamper.
- [x] Preserve `pilot-b.integrity.v3`, qualification/fingerprint semantics, preflight/ownership behavior, and all #48/#49/#50, protocol/schema, WPF, real CLI/auth boundaries.
- [x] Complete full Release validation and exact-scope/adversarial review; synchronize all directly verified #45 acceptance checkboxes and publish lifecycle `VERIFIED` without closure or Git publication.

RED evidence: the publisher type was initially absent; runner injection initially resolved to the unrelated ownership-checkpoint constructor; undeclared pre-seal content allowed final seal publication; reserved temp/final collisions were intercepted before native create-new/rename semantics; and a canonical hard-linked payload incorrectly re-verified as `SEALED`. Spec review then reproduced six further fail-closed gaps: reparse bundle root, extra/duplicate seal properties, extra/duplicate metadata properties, and metadata byte drift before seal publication. Each failure was observed before its minimal fix. Current GREEN focused checkpoint: publisher plus existing runner tests 75/75. The single-read verifier invariant remains structural and is not tested with a race or fault framework.

Final verification: full PilotB Release tests passed 143/143; Release solution tests passed 283/283 (19+10+3+22+143+71+15); Release build completed with 0 warnings and 0 errors. Tracked `git diff --check` passed and both untracked no-index whitespace checks emitted no diagnostics. Exact scope is six authorized files: bundle/publisher/runner, focused publisher tests, harness, and this todo section. Standards review's suggested mutation-test abstraction was rejected as unnecessary; `tasks/lessons.md` remained untouched under the exact-file boundary. Spec review's root-reparse, exact-schema, pre-seal metadata, and snapshot-reuse findings were fixed and the final re-review reported no remaining defect. Residual boundaries are deliberate: no concurrent race/fault framework, no durability claim beyond `Flush(true)` best effort, and native hard-link-count enforcement is Windows-only. No stage, commit, push, PR, merge, Issue closure, real CLI/auth, or Pilot B execution was performed.

## Pilot B v3 Issue #48 — caller cancellation and process-tree termination (active)

- [x] Fetch and confirm exact `main@7490859`, create the clean isolated worktree/branch, re-read live #48/#31/#43/#49/#50, pass the 27/27 focused Release baseline, and publish lifecycle `STARTED`.
- [x] Add deterministic fake-CLI regressions with bounded readiness/PID polling for cancellation before output, during flushed output, with an owned child process, and with injected termination failure.
- [x] Separate caller cancellation from runner timeout, add one narrow internal process-tree terminator, await/close capture resources, retain termination failure as secondary OCE context, and abandon final publication.
- [x] Complete adversarial scope/race review plus focused/full Release tests, Release build, whitespace checks, and exact-file audit.
- [x] Synchronize only directly verified #48 acceptance checkboxes and publish lifecycle `VERIFIED` or `PARTIAL`; stop before stage, commit, push, PR, merge, or Issue closure.

Preflight evidence: remote and local `origin/main` both resolved to `7490859ec8019c7faa6999f55ecc7645bb434e88` with GitHub check rollup `SUCCESS`; #48 was OPEN/`ready-for-agent`, 0/7, with closed blocker #43. The new worktree was clean at that exact SHA, and the initial focused `PilotBRunnerTests` baseline passed 27/27 using the existing repo-local .NET 8.0.421 executable only as the SDK host.

RED/GREEN evidence: the first sole-parent tracer passed under the old code because cancellation could race into the shared timeout task and accidentally execute the timeout kill. The injected-failure regression then failed to compile because no cancellation-specific termination seam existed. After separating caller cancellation from timeout, adding `IPilotBProcessTreeTerminator`, and giving capture tasks a runner-owned cancellation source, the first injected-failure GREEN attempt exposed a blocked pipe copy; cancelling that capture source on termination failure fixed the hang without masking the caller OCE. Final focused checkpoint is 33/33: 27 existing runner tests plus four required cancellation cases, bounded non-termination failure, and already-exited process handling. Existing timeout tests remain green and still require `SEALED + INVALID` with the `timeout` reason and no false `nonzero-exit`.

Final observable behavior: normal cancellation rethrows an OCE carrying the caller token after the fake parent/child handles exit; injected exception and bounded non-termination failure are retained in `OCE.Data`, and each failure test owns fallback process-tree cleanup. Cancellation leaves the artifact directory with already-written diagnostic payloads, no metadata, `integrity.json.tmp`, `integrity.json`, fingerprint, or retained write lock. PID plus process-start identity prevents marker reuse, and a post-suite process scan found no fake-CLI process.

Final validation: full PilotB Release tests passed 149/149; Release solution tests passed 289/289 (19+10+3+22+149+71+15); Release build completed with 0 warnings and 0 errors. Tracked `git diff --check` passed and untracked no-index checks emitted no whitespace diagnostics. Exact scope is six authorized files: runner, one narrow terminator, fake CLI, focused cancellation tests, harness, and this todo section.

Adversarial review fixed unbounded termination/capture waiting, the non-OCE cancellation race, PID-reuse exposure, unnamed test timing constants, the already-exited proof gap, and stale fault-seam wording. A cancellation-vs-timeout simultaneous timer race remains proven structurally rather than through a new production hook or flaky timing test: the caller token is checked before timeout classification and again after process exit/capture, while existing timeout tests remain green. Atomic seal rename remains the publication commit point and is not rolled back on a later token signal. Detached-child Job Object tracking is outside this narrow `Kill(entireProcessTree: true)` slice; the canonical fake parent/child tree is directly proven terminated. `tasks/lessons.md` was not changed because the exact Issue #48 file boundary excludes it. No stage, commit, push, PR, merge, Issue closure, real CLI/auth, or Pilot B execution was performed.

GitHub lifecycle: Issue #48 acceptance was re-read at 7/7 and the final `VERIFIED` evidence was published at `issuecomment-5482259039`; #48 and parent #31 remain OPEN.

## Pilot B v3 Issue #49 — controlled failures as sealed invalid (active)

- [x] Verify live #49/dependencies and exact remote `main@748f5eb`, create the clean isolated worktree/branch, pass the 149/149 Pilot B Release baseline, and publish lifecycle `STARTED`.
- [x] Add deterministic fake-CLI coverage for malformed, partial, unsupported, out-of-order, terminal-failure, clean-transcript nonzero, and timeout outcomes with exact ordered deduplicated reasons.
- [x] Reuse the existing process-tree terminator for runner timeout; prove successful parent/child termination seals invalid evidence and exception/non-termination returns bounded `UNSEALED + null` with test-owned cleanup.
- [x] Prove executable, prompt, and repository-boundary drift plus runner/metadata/seal/verifier agreement through disposable files and the existing publisher seam.
- [x] Complete full Release validation, no-fake-process check, exact-scope/adversarial review, and final harness evidence.
- [x] Synchronize only directly verified #49 acceptance checkboxes and publish lifecycle `VERIFIED` or `PARTIAL`; stop before stage, commit, push, PR, merge, or Issue closure.

Preflight evidence: #49 was OPEN/`ready-for-agent`, 0/7, with no comments; blocker #43 and prerequisite #48 were CLOSED/completed; #50 remained OPEN and blocked by #49. GitHub and the local object database both resolved `main=748f5eb650f365be84715144bcb8a212dda080b2`. The target path and branch were absent, the new worktree was clean at that SHA, and the initial Pilot B Release suite passed 149/149 using the existing repo-local .NET 8 SDK host after one environment-only restore.

RED/GREEN evidence: unsupported, out-of-order, isolated terminal-failure, and clean-transcript nonzero prompts first resolved through the fake CLI default and failed their exact reason vectors before dedicated deterministic modes were added. Timeout first bypassed the injected process-tree terminator, and an injected timeout-termination exception incorrectly produced `SEALED`; both failures were reproduced before the minimal bounded fail-closed implementation. Prompt drift first returned `UNSEALED` because verifier reconstruction compared captured bytes to a captured hash rather than the original prompt hash; the existing `prompt_sha256` field now retains the original exact prompt-byte hash while payload inventory independently binds the captured bytes. Current focused runner/cancellation/controlled-failure checkpoint: 48/48 passed. No parser, contracts, schema/protocol, run-record, scorer, WPF, config, dependency, real CLI/auth, Git publication, or Issue closure change was made.

Final verification: focused runner/cancellation/controlled-failure tests passed 48/48; full Pilot B Release tests passed 164/164; Release solution tests passed 304/304 (19+10+3+22+164+71+15); Release build completed with 0 warnings and 0 errors. Tracked `git diff --check` passed and the untracked no-index whitespace check emitted no diagnostics. No fake-CLI process remained. Exact scope is seven authorized files: runner, evidence verifier, fake CLI, controlled-failure and cancellation tests, focused harness, and this todo section. Final Standards and Spec re-reviews reported no actionable findings. Residual boundaries are deliberate: no real CLI/auth or experiment was run; detached-child Job Object tracking and stronger hardware-durability claims remain out of scope; `tasks/lessons.md` remains untouched under the exact-file boundary.

GitHub lifecycle: Issue #49 acceptance was re-read at 7/7 and the final `VERIFIED` evidence was published at `issuecomment-5485997803`; #49 and parent #31 remain OPEN. The worktree stops at the reviewed, verified, uncommitted seven-file diff required by the Goal contract.

### PR #56 CodeRabbit capture-lifetime remediation

- [x] Re-read live Issue #49, PR #56, exact head SHA, CI, and the current unresolved CodeRabbit thread before edits; publish one scoped lifecycle `PROGRESS` update.
- [x] Reuse one shared stdout/stderr capture task, snapshot only after completion, and defer disposal of capture-owned buffers/token source until outstanding writers settle.
- [x] Preserve caller-cancellation and controlled-timeout fail-closed behavior through the existing 10/10 cancellation and 48/48 combined runner regression matrices.
- [x] Pass full Pilot B and solution Release tests/build, whitespace/process/scope gates, and final Standards/Spec review.
- [x] Publish exact lifecycle verification and prepare the CodeRabbit reply for the post-push CI checkpoint.

Finding and implementation evidence: on PR head `09dd3f8de7bcde9e0133b8be24ae3c4ccc55dd49`, bounded `WaitAsync` in capture abort could return while `CopyToAsync` still owned a destination `MemoryStream`; the old lexical `using` scope then allowed concurrent snapshot/disposal. The runner now creates one shared completion task for both pipes, passes it through cancellation and timeout termination, snapshots only when it is complete, and transfers buffer/token cleanup to a private completion-bound helper. The helper owns no process, artifact, publisher, qualification, or sealing state. The existing `processTerminationCompleted` publication gate and `UNSEALED + null` failure result are unchanged; no new fault seam or timing race test was added.

Final verification: cancellation tests passed 10/10; the combined runner/cancellation/controlled-failure matrix passed 48/48; full Pilot B Release passed 164/164; solution Release passed 304/304 (19+10+3+22+164+71+15); Release build completed with 0 warnings and 0 errors. `git diff --check` passed, the worktree has no untracked files, no fake-CLI process remained, and the exact follow-up scope is four files: runner, focused harness, todo, and lessons. Structural review confirmed that no snapshot or disposal can precede capture-task completion and that the existing seal gate is unchanged; final Standards and Spec reviews found 0 actionable issues. Lifecycle `VERIFIED` evidence was published at `issuecomment-5507480299`; the review-thread reply remains a post-push action and requires no repository change.

### PR #56 process-backed test serialization

- [x] Re-read live Issue #49, PR #56, exact head `91a1015`, repeated Windows CI failures, and clean worktree; publish the authorized test-only lifecycle `PROGRESS` update.
- [x] Put every process-backed Pilot B runner test class in one xUnit collection without changing production code, behavioral timeouts, or assertions.
- [x] Run focused process-backed tests, full Pilot B and solution Release tests/build, whitespace/process/scope gates, and final Standards/Spec review.
- [x] Review the exact separate test-only commit boundary and record the required post-commit memory gate, normal push to PR #56, and one fresh complete CI result.

RED evidence is the repeated PR #56 `windows-latest` run `33615262892`: two attempts reproduced strict normal-run timing misclassification and a process-marker timeout while the exact failed cases and full 164/164 Pilot B suite passed locally. This follow-up changes only test scheduling: the five classes that execute or coordinate the fake CLI now share one collection and therefore cannot overlap each other. Pure non-process Pilot B tests retain normal xUnit parallelism.

GREEN evidence: the complete process-backed collection passed 111/111; full Pilot B Release passed 164/164; solution Release passed 304/304 (19+10+3+22+164+71+15); Release build completed with 0 warnings and 0 errors. Tracked and new-file whitespace checks passed, no FakeCli process remained, and the exact scope is six test-infrastructure files plus this task log and one reusable lesson. No production file, timeout value, assertion, schema/protocol, qualification, verifier, scorer, WPF, configuration, dependency, real CLI/auth, or Issue #50 behavior changed. Final Standards and Spec reviews found no actionable issue.

## Pilot B v3 Issue #50 — verified scoring handoff and semantic repeatability (verified locally; lifecycle synced)

- [x] Confirm live #50/#30/#31 and closed blockers #45/#46/#47/#48/#49; create the clean isolated worktree at exact `main@3f842797`; pass the 164/164 Pilot B Release baseline; publish lifecycle `STARTED`.
- [x] Add deterministic RED coverage for verifier-owned evidence projection, producer acceptance/rejection, exact commentary projection, and JSONL round-trip.
- [x] Implement the minimal verifier-owned immutable projection, remove duplicate runner scoring eligibility, and add a fail-closed `PilotBRunRecordProducer` that re-verifies immediately before projection.
- [x] Prove semantic fingerprint and canonical score repeatability across storage-only variation without changing frozen schemas, fingerprint bytes, or scorer semantics.
- [x] Run focused/full Release validation, no-FakeCli and whitespace gates, exact-scope adversarial review, documentation sync, and lifecycle `VERIFIED`/`PARTIAL` without Git publication or Issue closure.

Preflight evidence: remote and local `origin/main` resolve to `3f842797f2311d938a41c8a50acb707e20a90286`; #50 is OPEN/`ready-for-agent`, parents #30/#31 remain OPEN, and blockers #45/#46/#47/#48/#49 are CLOSED/completed. The new `codex/pilot-b-harness-scoring-handoff` worktree is clean at that exact SHA. Environment-only restore used the existing repo-local .NET 8 SDK; the initial Pilot B Release baseline passed 164/164. Lifecycle `STARTED` was published at `issuecomment-5509212383`.

RED/GREEN evidence: the first focused run failed to compile because no typed verified projection, producer request/result, or rejection contract existed. The minimal implementation makes verifier output the sole immutable downstream snapshot, routes every sealed runner field through it, removes `PilotBRunnerResult.IsScored`, and re-verifies inside the producer before exact commentary projection. Typed rejection covers unsealed tamper, sealed-invalid, qualification, and malformed message-kind cardinality without artifact mutation. Golden bytes lock the existing `pilot-b.run-fingerprint.v3`; integrated final-before-commentary coverage retains global sequences 2/3 and excludes final from primary messages. Independently republished storage variants change raw output, stderr, and arm-manifest hashes plus physical roots/timestamps/IDs while preserving the semantic fingerprint and canonical scorer result. Current focused Release checkpoint: 137/137.

Final verification: the exact contract filter passed 136/136 and the separated fingerprint-writer golden passed 1/1; the expanded focused set passed 137/137. Full Pilot B Release passed 173/173. Solution Release passed 313/313 (19 Domain + 10 Application + 3 LiveDryRun + 22 Desktop + 71 Infrastructure + 173 Pilot B + 15 Memory), and the Release build completed with 0 warnings and 0 errors. Tracked and untracked whitespace checks passed, no PilotB/FakeCli process remained, and the reviewed diff is exactly nine authorized files. Independent Standards and Spec reviews found no actionable issue. Frozen run-record/fingerprint schemas and scorer behavior remain unchanged; real CLI/authentication, protocol freeze, adjudication, primary Pilot B execution, Git publication, and closure of #50/#30/#31 remain explicit human/authority gates.

GitHub lifecycle checkpoint: the live Issue #50 body was re-read, exactly ten directly verified acceptance checkboxes were updated, and the read-back confirmed OPEN with 10/10 checked. After fresh user authorization, the sanitized `VERIFIED` comment was published and read back at `issuecomment-5510360905`; #50, #30, and #31 remain OPEN.

### PR #57 cancellation cleanup investigation

- [x] Confirm clean `ae86bac23e6fa1702ff8fed604dd1659e747bcb1`, OPEN/DRAFT PR #57, OPEN Issue #50, and closed dependencies.
- [x] Inspect the installed runtime and native process-handle signaling after the existing cleanup wait; instrument separate primary/cleanup exception output during the experiment.
- [x] Report the evidence gap without a speculative fix: no unsignaled handle or locked-directory failure was observed in the valid diagnostic run.
- [x] Run existing focused cancellation coverage, full Pilot B and solution Release tests/build, whitespace and scope review; retain local PARTIAL evidence without commit/push.
- [ ] Publish lifecycle PARTIAL to Issue #50 after the blocked external mutation is explicitly cleared; do not close the issue or claim the CI defect fixed.

Boundary: no production change, timeout inflation, retry-delete, broad serialization, new dependency, real CLI/auth, or Git publication. The existing runner/fake-CLI boundary is the diagnostic seam; race observations are not permanent probabilistic regression tests. Local runtime is .NET 8.0.27 (SDK 8.0.421); the prior CI log records SDK 8.0.424 but not the selected test runtime. Lifecycle STARTED was blocked by the connector action guard's Issue/PR target ambiguity; no comment was published.

Diagnostic evidence: baseline cancellation 10/10. A disposable IL probe against the installed `System.Diagnostics.Process` assembly identified `8.0.27+a6bde67c455f2ac219988c7a66171631090b6f65` and confirmed the exit-code fast path in `ProcessManager.HasExited`, the async bypass of the event wait, and the direct native-handle wait in `WaitForExitCore`. This establishes a possible mechanism, not the CI cause. In the valid instrumented 10/10 cancellation run, all ten cleanup observations were signaled (four after async wait, six early-return paths); both controlled-timeout fault tests had signaled handles and completed runner tasks before successful fixture deletion. For both `WhenTerminationThrows` and `WhenProcessDoesNotStop`, neither the test-body exception logger nor the cleanup exception logger was triggered; the deliberately injected runner termination failures remained expected test inputs. An earlier instrumentation attempt failed 4/10 because it opened handles after process exit; that measurement defect was corrected by pinning identity while alive and is not product RED evidence. All temporary test instrumentation was then removed.

Remaining gap: the exact CI test runtime, original exception (if masked by disposal), and lock owner at the CI failure instant are unavailable in the existing log; that run has no uploaded artifacts. No deterministic RED regression or causal correction is claimed. A next CI diagnostic slice would need failure-time primary/cleanup output, a handle pinned before termination, actual runtime metadata, and retained diagnostics. Do not infer an external lock owner merely from local signaled-and-successful cleanup. No additional CI run or publication is authorized here.

Final local validation after removing instrumentation: cancellation 10/10; Pilot B 173/173; solution 313/313 (19 Domain + 10 Application + 3 LiveDryRun + 22 Desktop + 71 Infrastructure + 173 Pilot B + 15 Memory); Release build 0 warnings/0 errors; `git diff --check` passed. Production and test source match HEAD; only this scoped log and one lesson are modified. No new regression test, cleanup correction, commit, memory refresh, push, PR mutation, or CI rerun occurred. Investigation status remains PARTIAL; GitHub sync remains BLOCKED for the attempted lifecycle write.

Documentation review: Spec found no actionable finding; Standards requested clearer separation of observed test-body/cleanup exceptions from deliberately injected runner failures, and that wording was clarified. The next action is a separately scoped diagnostic CI change, not an unproven wait replacement or a blind rerun.

### PR #57 diagnostic-only CI follow-up (Issue #50)

- [x] Recheck unchanged `ae86bac`, OPEN/DRAFT PR #57, OPEN Issue #50 and closed dependencies; preserve the two prior investigation documentation edits.
- [x] Add bounded diagnostics to the two controlled-timeout fault tests: exact test runtime, independent primary/cleanup/disposal exceptions, and a pinned native-handle observation at failure.
- [x] Add deterministic diagnostic checks; confirm runtime/handle output is retained in local TRX without changing existing test selection or outcomes.
- [ ] Retain CI TRX results on failure: the proposed GitHub Actions artifact-upload change was rejected by the action guard pending explicit permission to export exception/path diagnostics to this repository for seven days. Workflow remains unchanged.
- [x] Validate cancellation, full Pilot B, solution tests/build, whitespace and exact scope; review diagnostic non-interference.
- [ ] Obtain failure-time CI evidence before any causal fix or fix commit.

Boundary: diagnostics only; retain current kill/wait/observe/dispose sequence, time budgets, production code, test collection and assertions. No retry-delete, fault framework, new dependency, real CLI/auth, commit, push or rerun. The previously rejected Issue #50 lifecycle mutation remains BLOCKED pending explicit clearance; local work may continue. Diagnostic publication to existing PR #57 requires a separately authorized diagnostic commit/push; it is not a cleanup fix.

Local evidence: baseline cancellation 10/10. Two deterministic diagnostics tests started with compile RED (missing helper); their first implementation run was 11/12 because the new assertion compared the entire exception string before versus after rethrow. It now checks the exact type/message and original fixture-disposal stack frame, allowing only additional rethrow frames. Focused diagnostic/cancellation verification passed 12/12. The real locked-file test proves that a primary exception stays recorded separately from a subsequently rethrown disposal IOException; a current-process test proves the retained handle remains observable after Process.Dispose and cleanup-return output is buffered until fixture deletion.

New observation: one successful pre-buffering focused run recorded `WAIT_TIMEOUT (0x102)` immediately after the existing cleanup helper returned in `WhenTerminationThrows`, followed by exit code `0xffffffff`; directory deletion nevertheless succeeded. These are adjacent non-atomic native observations, not a proven cause of the CI directory lock. Output at this checkpoint is now buffered to avoid output latency before deletion. The buffered focused run passed 12/12 with both fault-test handles signaled. Actual local runtime is .NET 8.0.27, Process assembly `8.0.27+a6bde67c455f2ac219988c7a66171631090b6f65`; actual CI runtime and failure-time state remain unknown. No wait replacement or root-cause fix is claimed.

Pre-review validation: focused 12/12 (10 existing cancellation + 2 diagnostic checks); full Pilot B 175/175; solution 315/315 (19 Domain + 10 Application + 3 LiveDryRun + 22 Desktop + 71 Infrastructure + 175 Pilot B + 15 Memory); Release build 0 warnings/0 errors. Tracked diff and new-file whitespace checks passed; no FakeCli remained. Production and CI workflow match HEAD. Local TRX `cancellation-diagnostics-buffered_net8.0_20260902212105.trx` and `pilotb-diagnostics_net8.0_20260902212237.trx` under ignored `tools/PilotB.Tests/TestResults/` were parsed to confirm actual runtime and handle output.

Review: the read-only reviewer identified output-sink failure and duplicate buffered-output risks; diagnostics now isolate sink errors from teardown and emit the buffered snapshot only once. The primary-masking concern is handled by preserving the original exception in diagnostic output, not changing the existing exception precedence: a deterministic test now throws an active primary exception followed by a real locked-file disposal failure and asserts that both remain recorded. Added a third test proving output failure cannot prevent fixture cleanup. A temporary compile error selected xUnit's forbidden async overload for the all-throw lambda; an explicit synchronous Action fixed that test expression. Final focused validation is 13/13, with native/runtime output checked in `cancellation-diagnostics-final_net8.0_20260902212851.trx`.

Final post-review verification: focused 13/13; full Pilot B 176/176; solution 316/316 (19 Domain + 10 Application + 3 LiveDryRun + 22 Desktop + 71 Infrastructure + 176 Pilot B + 15 Memory); Release build 0 warnings/0 errors. No existing assertion, timeout, wait/kill helper, production file or CI workflow changed. Diff contains exactly five files including the two prior documentation edits; no generated TRX is tracked. Final scope/whitespace and process-leak checks passed. Native observations remain diagnostic-only, with nonzero sampling overhead and no claim of reproducing the CI directory-lock failure.
...
- [x] Live preflight: HEAD/PR head remains `ae86bac23e6fa1702ff8fed604dd1659e747bcb1`; PR #57 OPEN/DRAFT, Issue #50 OPEN, dependencies #45/#46/#47/#48/#49 CLOSED/COMPLETED; exactly five prepared local files, no unrelated changes.
- [x] Add one CI upload step restricted to `tools/PilotB.Tests/TestResults/*.trx`, seven-day retention and run/attempt-specific name; run after success/failure unless cancelled, without altering test commands or outcomes.
- [x] Verify workflow and prepared source; exact-path commit boundary is six files with `test(pilot-b): capture cancellation cleanup diagnostics`.
- [ ] Memory status, refresh committed HEAD when needed, repeat status and pre-push gate; normal push to `codex/pilot-b-harness-scoring-handoff` and verify PR head/CI dispatch.

Boundary: publication authorization supersedes the earlier no-commit/no-push and artifact-export gates only. Production behavior, cleanup order, time budgets and assertions remain unchanged. No root-cause fix, new PR, ready-for-review transition, merge or Issue closure. The prior Issue #50 comment rejection remains `GITHUB_SYNC_BLOCKED`; no lifecycle mutation retry is authorized by this publication checkpoint. CI failure-time evidence is still required before selecting a causal fix.

Publication pre-commit verification: focused 13/13; full solution 316/316, including PilotB 176/176; Release build 0 warnings/0 errors. YAML parsing and structural comparison against HEAD prove that the sole workflow change is the approved upload step; existing jobs, permissions, triggers and test commands are unchanged. The first YAML check could not read the installed PyYAML package in the sandbox; the same read-only check passed outside it, without dependency installation. Fresh ignored `diagnostic-publication_net8.0_20260902215801.trx` confirms .NET 8.0.27, Process assembly metadata and signaled native handles in both fault tests; fixture deletion succeeded. Diff whitespace and production no-change checks pass; no FakeCli remains. Next operations are the authorized commit, committed-HEAD memory gate and normal push; record their exact SHA/results in the handoff, without a second bookkeeping commit.

### PR #57 primary-failure preservation follow-up (Issue #50)

- [x] Live preflight: clean local/PR HEAD `50a18cb6a990661d3e78f6daa2c10a9847d9b0ea`, PR OPEN/ready, Issue #50 OPEN with existing 10/10 checklist, dependencies #45/#46/#47/#48/#49 CLOSED. Lifecycle STARTED posted.
- [x] Prove deterministic RED: a locked fixture returned IOException instead of the original InvalidOperationException; exact object assertion failed (1/1). Baseline focused tests passed 13/13.
- [x] Preserve first failure through runner cleanup and fixture disposal; diagnose subsequent errors, but propagate cleanup-only failures. Initial GREEN focused tests passed 13/13; expanded deterministic first-failure/disposal matrix passed 18/18 without race tests.
- [x] Focused/full PilotB/solution Release tests, Release build, native snapshots, exact-scope diff and Standards/Spec review.
- [ ] Separate exact-path commit, committed-HEAD memory status/refresh/status/pre-push gate, normal push to existing PR #57, CI/review and lifecycle evidence.

Boundary: only cancellation diagnostics, diagnostics tests, the two instrumented cancellation fault tests and scoped todo/lesson evidence. No production, native-wait, timeout, test collection, dependency, workflow/retention or cross-platform changes. Merge and Issue closure require separate authorization. Historical publication/sync checkpoints above are snapshots: the authorized Issue synchronization succeeded and PR became ready before this follow-up.

Local verification: focused 18/18 (10 cancellation + 8 diagnostic cases); full PilotB 181/181; full solution 321/321 (19 Domain + 10 Application + 71 Infrastructure + 22 Desktop + 3 LiveDryRun + 181 PilotB + 15 Memory). Release build: 0 warnings / 0 errors. Six fault-test samples across ignored `primary-preservation-{focused,pilotb,solution}*.trx` show native-wait `0x0`, runner `RanToCompletion`, no primary/cleanup/disposal errors and successful fixture deletion on local .NET 8.0.27. No PilotB/FakeCli/testhost remained after validation. Whitespace checks passed; production, native-wait helper, budgets, collection and workflow are unchanged.
